...
///
/// The iptables comment comes from `config.iptables_comment()` so the
/// `-D` calls only match this installation's rules even when another
/// ember install on the same host has rules for the same TAP/IP.
